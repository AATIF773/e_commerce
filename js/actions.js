$(document).ready(function(){
	...
	//publisher() is a funtion fetching publisher record from database whenever page is load
	function publisher(){
		$.ajax({
			url	:	"action.php",
			method:	"POST",
	...
	*/
	...
	/*	when page is load successfully then there is a list of publishers when user click on publisher we will get publisher id and 
		according to publisher id we will show products
	*/
	$("body").delegate(".selectPublisher","click",function(event){
		event.preventDefault();
		$("#get_product").html("<h3>Loading...</h3>");
		var bid = $(this).attr('bid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{selectPublisher:1,publisher_id:bid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
