...
if(isset($_POST["publisher"])){
	...
	$run_query = mysqli_query($con,$publisher_query);
	echo "
		<div class='aside'>
	...
	if(mysqli_num_rows($run_query) > 0){
		...
		while($row = mysqli_fetch_array($run_query)){
			...
			$bid = $row["publisher_id"];
			$publisher_name = $row["publisher_title"];
            $sql = "SELECT COUNT(*) AS count_items FROM products WHERE product_publisher=$i";
            $query = mysqli_query($con,$sql);
            $row = mysqli_fetch_array($query);
            $count=$row["count_items"];
            $i++;
			echo "
                    ...
                    <div type='button' class='btn navbar-btn selectPublisher' bid='$bid'>
...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			$pro_publisher = $row['product_publisher'];
                        ...
                        <div class='col-md-4 col-xs-6' >
								<a href='product.php?p=$pro_id'><div class='product'>
									<div class='product-img'>
										<img src='product_images/$pro_image' style='max-height: 280px;' alt=''>
									...
									<div class='product-body'>
										...
										<h4 class='product-price header-cart-item-info'>₹$pro_price<del class='product-old-price'> ₹$product_old_price</del></h4>
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectPublisher"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["cat_id"];
		$sql = "SELECT * FROM products,categories WHERE product_cat = '$id' AND product_cat=cat_id";
        
	}else if(isset($_POST["selectPublisher"])){
		$id = $_POST["publisher_id"];
		$sql = "SELECT * FROM products,categories WHERE product_publisher = '$id' AND product_cat=cat_id";
	}else {
